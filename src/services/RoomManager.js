...
class RoomManager {
  ...
  joinRoom(roomId, userId, userInfo, password = null) {
    ...
    if (!room) {
      room = this.createRoom(userId, { requirePassword: !!password, password });
      // Remove the auto-generated UUID and use provided roomId
      const autoGeneratedId = room.roomId;
      this.rooms.delete(autoGeneratedId);
      room.roomId = roomId;
      this.rooms.set(roomId, room);
      logger.info(`Auto-created room: ${roomId} (was ${autoGeneratedId})`);
    }
